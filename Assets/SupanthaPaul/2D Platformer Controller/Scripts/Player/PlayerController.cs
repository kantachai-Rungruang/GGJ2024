...
namespace SupanthaPaul
{
	public class PlayerController : MonoBehaviour
	{
		...
		public Vector3 grabRightOffset = new Vector3(0.16f, 0f, 0f);
		public Vector3 grabLeftOffset = new Vector3(-0.16f, 0f, 0f);
		public float grabCheckRadius = 0.24f;
		public float slideSpeed = 2.5f;
		public Vector3 wallJumpForce = new Vector3(10.5f, 18f, 0f);
		public Vector3 wallClimbForce = new Vector3(4f, 14f, 0f);
		...
		private void FixedUpdate()
		{
			...
			// check if on wall
			m_onWall = Physics.OverlapSphere((Vector3)position + grabRightOffset, grabCheckRadius, whatIsGround).Length > 0
			          || Physics.OverlapSphere((Vector3)position + grabLeftOffset, grabCheckRadius, whatIsGround).Length > 0;
			m_onRightWall = Physics.OverlapSphere((Vector3)position + grabRightOffset, grabCheckRadius, whatIsGround).Length > 0;
			m_onLeftWall = Physics.OverlapSphere((Vector3)position + grabLeftOffset, grabCheckRadius, whatIsGround).Length > 0;
			...
			if (isCurrentlyPlayable)
			{
				...
				if(m_wallJumping)
				{
					m_rb.velocity = Vector3.Lerp(m_rb.velocity, (new Vector3(moveInput * speed, m_rb.velocity.y)), 1.5f * Time.fixedDeltaTime);
				}
				else
				{
					if(canMove && !m_wallGrabbing)
						m_rb.velocity = new Vector3(moveInput * speed, m_rb.velocity.y, 0f);
					else if(!canMove)
				...
				if (isDashing)
				{
					...
					else
					{
						...
						if(m_facingRight)
							m_rb.velocity = Vector3.right * dashSpeed;
						else
							m_rb.velocity = Vector3.left * dashSpeed;
					}
				}
				...
				if(m_onWall && !isGrounded && m_rb.velocity.y <= 0f && m_playerSide == m_onWallSide)
				{
					...
					m_wallGrabbing = true;
					m_rb.velocity = new Vector3(moveInput * speed, -slideSpeed, 0f);
		...
		private void Update()
		{
			...
			{
				m_rb.velocity = new Vector3(m_rb.velocity.x, m_extraJumpForce, 0f);
				m_extraJumps--;
				// jumpEffect
			...
			{
				m_rb.velocity = new Vector3(m_rb.velocity.x, jumpForce, 0f);
				// jumpEffect
				PoolManager.instance.ReuseObject(jumpEffect, groundCheck.position, Quaternion.identity);
			}
			else if(InputSystem.Jump() && m_wallGrabbing && moveInput!=m_onWallSide )		// wall jumping off the wall
			{
				...
				if (m_playerSide == m_onWallSide)
					Flip();
				m_rb.AddForce(new Vector3(-m_onWallSide * wallJumpForce.x, wallJumpForce.y, 0), ForceMode.Impulse);
			}
			else if(InputSystem.Jump() && m_wallGrabbing && moveInput != 0 && (moveInput == m_onWallSide))      // wall climbing jump
			{
				...
				if (m_playerSide == m_onWallSide)
					Flip();
				m_rb.AddForce(new Vector3(-m_onWallSide * wallClimbForce.x, wallClimbForce.y, 0), ForceMode.Impulse);
			}
		...
		private void OnDrawGizmosSelected()
		{
			...
			Gizmos.DrawWireSphere((Vector3)transform.position + grabRightOffset, grabCheckRadius);
			Gizmos.DrawWireSphere((Vector3)transform.position + grabLeftOffset, grabCheckRadius);
		}
	}
}
